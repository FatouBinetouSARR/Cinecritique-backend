...
// CORS
app.use(cors({
  origin: isProduction ? FRONTEND_ORIGIN : "http://localhost:5173",
  credentials: true,
}));
...
app.post("/api/auth/register", async (req, res) => {
  ...
  if (await User.findOne({ email: normalizedEmail })) return res.status(409).json({ message: "Utilisateur déjà existant" });
  ...
  try {
    ...
    res.status(201).json({ user: { id: newUser._id, email: normalizedEmail }, accessToken });
  } catch (err) {
...
app.post("/api/auth/refresh", async (req, res) => {
  try {
    ...
    console.log("💾 Token stocké trouvé :", storedToken);
